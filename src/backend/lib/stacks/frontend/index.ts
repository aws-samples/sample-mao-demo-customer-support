...
export class FrontendStack extends CommonStack {
    ...
    constructor(scope: Construct, id: string, props?: StackProps) {
        ...
        const cloudfrontWebAcl = new CloudfrontWebAcl(this, "cloudfrontWebAcl2", {
            managedRules: [
                {
                    vendor: "AWS",
...
export class FrontendDeploymentStack extends CommonStack {
    constructor(scope: Construct, id: string, props: FrontendDeploymentStackProps) {
        ...
        // The build (LabsReactProject) syncs `dist/` to the website bucket and
        // then invalidates CloudFront (in that order) — see codebuild.ts. It needs
        // the bucket name and write access; CodeBuild artifacts-to-S3 are not used.
        ...
        props.websiteBucket.grantReadWrite(reactProject);
        ...
        NagSuppressions.addResourceSuppressions(
            ...
            [
                ...
                {
                    id: "AwsSolutions-SF1",
                    reason: "The waiter state machine is auto-created by the CDK custom-resource Provider framework; its logging configuration is not user-configurable.",
                },
            ],
            true
        );
        ...
        // CloudFormation caps output/export values at 1024 characters. Large payloads
        // (e.g. VITE_RUNTIME_CONFIG, a multi-KB JSON blob) are delivered to the build
        // exclusively via the CodeBuild environment variable above, so we skip emitting
        // an informational stack output for any value that would exceed the CFN limit.
        const CFN_OUTPUT_MAX_LENGTH = 1024;
        Object.entries(props.environmentVariables).forEach(([key, value]) => {
            if (value.length > CFN_OUTPUT_MAX_LENGTH) {
                return;
            }
