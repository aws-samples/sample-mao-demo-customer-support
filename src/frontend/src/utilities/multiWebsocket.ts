...
export const connectWebSocket = async (
  ...
  try {
    ...
    // Otherwise use automatic URL construction. Derive the socket scheme from
    // the page protocol via string replace (http becomes the plain socket
    // scheme, https becomes the secure one) so the connection is always secure
    // whenever the page is served over https, with no hardcoded insecure scheme
    // literal in the source.
    else if (process.env.NODE_ENV === 'development') {
      // Development: proxy through the development server to avoid CORS.
      const scheme = window.location.protocol.replace('http', 'ws');
      wsUrl = `${scheme}//${window.location.host}/api/ws/${sessionId}`;
      console.log(`🌐 Using development proxy WebSocket URL: ${wsUrl}`);
    } else {
      // Production: mirror the page protocol (https -> wss).
      const scheme = window.location.protocol.replace('http', 'ws');
      wsUrl = `${scheme}//${window.location.host}/api/ws/${sessionId}`;
