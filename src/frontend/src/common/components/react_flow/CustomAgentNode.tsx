...
// Shorten a model/inference-profile id for compact display on the node badge,
// e.g. "us.amazon.nova-premier-v1:0" -> "nova-premier".
const shortenModelId = (modelId: string): string => {
  ...
  // "claude-haiku-4-5", matching the other node badges.
...
const CustomAgentNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  // Resolve the foundation model backing this agent for a compact node badge
  // (Req 2.8, 8.10). Safe if the runtime config is unavailable (e.g. dev).
  let modelBadge: string | undefined;
  try {
    const modelId = getModelForNode(id);
    modelBadge = modelId ? shortenModelId(modelId) : undefined;
  } catch {
  ...
  // Role subtitle shown under the agent name (matches the reference layout).
  ...
  const roleLabel = (data.role as string) || roleLabelMap[data.type as string] || 'Collaborator';
  ...
  const hiddenHandle = { background: 'transparent', border: 'none', width: 1, height: 1, minWidth: 1, minHeight: 1 };
  ...
  return (
    <div
      ...
      style={{
        ...
        background: '#ffffff',
        boxShadow: selected ? `0 0 0 3px ${nodeColor}` : '0 2px 6px rgba(0,0,0,0.10)',
        width: 244,
        minWidth: 244,
        ...data.style
      }}
    >
      {/* Connection handles (kept functional but visually minimal) */}
      ...
      >
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <div
            style={{
              ...
              fontSize: '13px',
              lineHeight: 1.25,
              // Wrap only between words; never split a word like "Troubleshooting"
              // or "Recommendation" across lines.
              overflowWrap: 'normal',
              wordBreak: 'normal',
            }}
          >
          ...
          <div style={{ color: '#6b7280', fontSize: '13px', marginTop: '3px' }}>({roleLabel})</div>
          {modelBadge && (
            <div
        ...
        {(isProcessing || processingComplete) && (
          <div className="agent-status" style={{ color: nodeColor, flexShrink: 0 }}>
            {isProcessing ? '●' : processingComplete ? '✓' : ''}
          </div>
        )}
      </div>
