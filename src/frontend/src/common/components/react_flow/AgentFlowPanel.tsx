...
import CodeInterpreterNode from './CodeInterpreterNode';
import CodeInterpreterEdge from './CodeInterpreterEdge';
...
const nodeTypes = {
  ...
  codeInterpreter: CodeInterpreterNode,
};

const edgeTypes = {
  customEdge: CustomEdge,
  ciEdge: CodeInterpreterEdge,
};
...
export const AgentFlowPanel: React.FC<AgentFlowPanelProps> = ({ height = '100%', sessionId, modelId = 'us.amazon.nova-micro-v1:0' }) => {
  ...
  // Function to find the edge(s) that should animate for a given node.
  //
  // An agent's edge is the one leading INTO it (its incoming connection):
  //   - supervisor-agent  -> e-customer-supervisor  (User Question -> Supervisor)
  //   - each specialist   -> e-supervisor-sa*        (Supervisor -> specialist)
  //   - response          -> e-supervisor-response
  //
  // Using ONLY the incoming edge means each agent lights just its own connecting
  // line as it is activated. Previously the supervisor matched every edge whose
  // id contained "supervisor", so the moment the supervisor started it lit up all
  // of its outgoing edges to every specialist at once. Driving animation from the
  // incoming edge (combined with the staggered per-agent activation) makes the
  // lines light up one at a time, in the order the agents are actually called.
  ...
  const onNodeClick: NodeMouseHandler = useCallback((_event, node) => {
    try {
      ...
      // Governance nodes (guardrail/evaluation) are treated like agent nodes here so
      // clicking them opens the trace modal with their trace data (they use a
      // "-node" suffix rather than "-agent").
      const isAgentNode = node.id.includes('-agent') || node.id === 'routing-classifier' || node.id === 'supervisor-agent' || node.id === 'guardrail-node' || node.id === 'evaluation-node';
  ...
  const onInit = useCallback((reactFlowInstance: ReactFlowInstance) => {
    ...
    // Ensure fit view is run on initialization. Let fitView size the diagram to
    // fill the panel (a small padding keeps the nodes large); do not override with
    // a fixed zoom afterwards, which previously shrank the diagram.
    setTimeout(() => {
      reactFlowInstance.fitView({ padding: 0.08, includeHiddenNodes: false });
  ...
  useEffect(() => {
    if (reactFlowInstanceRef.current) {
      setTimeout(() => {
        reactFlowInstanceRef.current?.fitView({ padding: 0.08, includeHiddenNodes: false });
      }, 50);
    }
