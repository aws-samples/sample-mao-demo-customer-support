...
import { getModelForNode } from '../../../utilities/runtimeConfig';
import { getAgentColor } from './FlowUtils';
import './FlowComponents.css';

/**
 * Turn a Bedrock inference-profile id into a friendly display name, e.g.
...
export function friendlyModelName(modelId: string | undefined): string {
  ...
  let s = modelId;
  s = s.replace(/^[a-z]{2}\./, '');                          // region prefix (us./eu./apac.)
  s = s.replace(/^(amazon|anthropic|meta|cohere|mistral|ai21|deepseek)\./, ''); // provider
  s = s.replace(/-v\d+(:\d+)?$/, '');                        // version suffix -vN:M
  s = s.replace(/-\d{8}$/, '');                              // trailing date -YYYYMMDD
  s = s.replace(/:\d+$/, '');                                // any leftover :N
  const words = s.split('-').filter(Boolean).map((p) =>
    /^\d+$/.test(p) ? p : p.charAt(0).toUpperCase() + p.slice(1)
  );
  // Join, then collapse adjacent version digits like "4 5" -> "4.5".
...
const TraceGroup: React.FC<TraceGroupProps> = ({ traceGroup, hideTitle = false, autoExpand = false }) => {
  ...
  // Fallback model names by agent type, used only if the runtime config can't be
  // read. These mirror the backend's per-agent model assignment (config.py).
  const getFallbackLLM = (agentType: string | undefined): string => {
    if (!agentType) return 'Model';
  ...
  // Resolve the model name from the runtime config (source of truth) by this
  // trace group's node id; fall back to the static map if config is unavailable.
  const resolveModelName = (): string => {
    const nodeId =
      traceGroup.agentId ||
      (traceGroup.originalAgentType ? collaboratorToNodeId(traceGroup.originalAgentType) : '');
    if (nodeId) {
      try {
        const modelId = getModelForNode(nodeId);
        if (modelId) return friendlyModelName(modelId);
      } catch {
        /* runtime config not available — fall through to static map */
      }
    }
    return getFallbackLLM(traceGroup.originalAgentType);
  };

  // Create title with agent name and its actual model.
  const title = `${agentName} (${resolveModelName()})`;
        ...
        if (content.includes('### KNOWLEDGE BASE RESPONSE ###')) {
          ...
          return (
            <div className="kb-response">
              <h4 style={{marginTop: 0, color: '#FF5722', fontWeight: 'bold'}}>Knowledge Base Result</h4>
  ...
  const mapAgentTypeToColor = (agentType: string | undefined): string => {
    ...
    if (normalizedType.includes('super') || normalizedType === 'supervisor') {
    ...
    } else if (normalizedType.includes('eval')) {
      return '#7c3aed'; // Violet/purple for Evaluation (matches workflow node)
    } else if (normalizedType.includes('guardrail') || normalizedType.includes('policy')) {
      return '#0d9488'; // Teal/green for Guardrail (matches workflow node)
      ...
      return '#A52A2A'; // Brown for OrderManagementAgent (hex so alpha-hex styling works)
    }
