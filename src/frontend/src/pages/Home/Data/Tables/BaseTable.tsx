...
const BaseTable: React.FC<BaseTableProps> = ({ 
  ...
  const { refresh } = useDataAPI();
  const [refreshing, setRefreshing] = useState(false);
  const [selectedItems, setSelectedItems] = useState<any[]>([]);

  // Re-fetch all datasets, then keep a brief spinner so the action reads as
  // deliberate even though the mock source resolves instantly.
  const handleRefresh = () => {
    setRefreshing(true);
    refresh();
