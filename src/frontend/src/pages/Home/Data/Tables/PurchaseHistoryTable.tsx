...
const PurchaseHistoryTable: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await dataAPI.fetchPurchaseHistory();
        setPurchaseHistory(data);
      } catch (error) {
        console.error("Error loading purchase history data:", error);
        addFlashbarItem("error", "Failed to load purchase history data from S3. Please try again.");
      }
    };
    ...
    return (
      <Box textAlign="center" padding="l">
        ...
        <Box variant="p" padding={{ top: "s" }}>
          Loading purchase history data from S3...
        </Box>
      </Box>
    );
  }
  ...
  if (error) {
    return (
      <Box textAlign="center" padding="l" color="text-status-error">
        ...
        <Box variant="p" padding={{ top: "s" }}>
          Unable to load purchase history data from S3. Please try refreshing the page.
        </Box>
      </Box>
    );
  }
